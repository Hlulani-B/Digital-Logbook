...
| Unit tests          | Jest                         | Backend (4 services)      | CI on every push            |
| Coverage (backend)  | Jest `--coverage`            | Backend (4 services)      | CI badge                    |
| Coverage badges     | `scripts/generate-badges.js` | All services              | Run manually from repo root |
...
**What**: Unit testing framework used across all four backend services (auth-service, dashboard-service, profile-service, project-service).
...
- SVG badges generated by running `node scripts/generate-badges.js` from the repo root
