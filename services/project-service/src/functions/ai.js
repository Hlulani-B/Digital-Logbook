...
console.log(
  '[AI] HF_API_KEY:',
  process.env.HF_API_KEY ? 'SET (' + process.env.HF_API_KEY.slice(0, 8) + '...)' : 'NOT SET'
);
console.log(
  '[AI] OPENROUTER_API_KEY:',
  process.env.OPENROUTER_API_KEY
    ? 'SET (' + process.env.OPENROUTER_API_KEY.slice(0, 8) + '...)'
    : 'NOT SET'
);
console.log(
  '[AI] CEREBRAS_API_KEY:',
  process.env.CEREBRAS_API_KEY
    ? 'SET (' + process.env.CEREBRAS_API_KEY.slice(0, 8) + '...)'
    : 'NOT SET'
);
console.log(
  '[AI] GEMINI_API_KEY:',
  process.env.GEMINI_API_KEY ? 'SET (' + process.env.GEMINI_API_KEY.slice(0, 8) + '...)' : 'NOT SET'
);
console.log(
  '[AI] GROQ_API_KEY:',
  process.env.GROQ_API_KEY ? 'SET (' + process.env.GROQ_API_KEY.slice(0, 8) + '...)' : 'NOT SET'
);
...
function isRateLimited(err) {
  ...
  const message = (err?.error?.message || err?.message || JSON.stringify(err || {})).toLowerCase();
...
async function callOpenRouterModel(model, question) {
  ...
  console.log(
    '[AI][OpenRouter] Got response, length:',
    content.length,
    'preview:',
    content.slice(0, 100)
  );
...
async function callHFModel(model, question) {
  ...
  const content = response.choices?.[0]?.message?.content ?? '';
  console.log(
    '[AI][HuggingFace] Got response, length:',
    content.length,
    'preview:',
    content.slice(0, 100)
  );
...
async function callCerebrasModel(model, question) {
  ...
  const content = response.choices?.[0]?.message?.content ?? '';
  console.log(
    '[AI][Cerebras] Got response, length:',
    content.length,
    'preview:',
    content.slice(0, 100)
  );
...
export async function AI(question) {
  ...
  const chain = [
    { name: 'HuggingFace', models: HF_MODELS, fn: callHFModel, enabled: !!process.env.HF_API_KEY },
    {
      ...
      fn: callOpenRouterModel,
      enabled: !!process.env.OPENROUTER_API_KEY,
    },
    {
      name: 'Cerebras',
      models: CEREBRAS_MODELS,
      fn: callCerebrasModel,
  ...
  chain.forEach((p) =>
    console.log('[AI]  ', p.name, '- enabled:', p.enabled, 'models:', p.models.length)
  );
